...
Flag system for Gemini CLI SuperGemini framework with auto-activation and conflict resolution.
...
- Enable parallel processing delegation for complex operations
...
**`--concurrency [n]`**
- Control max concurrent operations (default: 5, range: 1-10)
...
- **off**: Disable wave mode, use delegation instead
...
- Control how Wave system delegates work
- **files**: Delegate individual file analysis across waves
- **folders**: Delegate directory-level analysis across waves
- **tasks**: Delegate by task type (security, performance, quality, architecture)
...
- Compatible commands: /sg:improve, /sg:refine, /sg:enhance, /sg:fix, /sg:cleanup, /sg:analyze
...
8. Delegation: explicit --delegate > auto-detection
...
**Delegation Auto-Activation**: >7 directories OR >50 files OR complexity >0.8
**Loop Auto-Activation**: polish, refine, enhance, improve keywords detected

## Gemini-Specific Considerations

### Tool Permission Flags
- **`--allow-all-tools`**: Enable all Gemini tools without confirmation
- **`--require-confirmation`**: Always ask before tool execution
- **`--sandbox`**: Run in sandboxed mode (default in some environments)

### Memory Management Flags
- **`--memory-context`**: Include memory context in operations
- **`--clear-memory`**: Clear memory before operation
- **`--preserve-memory`**: Maintain memory across operations

### Extension Compatibility
- All flags integrate with Gemini's extension system
- Flags can be configured in `gemini-extension.json`
- Project-specific flag defaults via `.gemini/settings.json`